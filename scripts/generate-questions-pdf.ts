...
const OUTPUT_PDF = path.join(OUTPUT_DIR, `questions-report-${new Date().toISOString().split('T')[0]}.pdf`);
...
async function generatePDF() {
  try {
    ...
    console.log('DDRC Questions PDF Generator');
    console.log('='.repeat(70));
    console.log();
    ...
    console.log(`[OK] Loaded ${sections.length} sections with ${sections.reduce((sum, s) => sum + s.questions.length, 0)} total questions`);
    console.log();

    console.log('[INFO] Generating HTML...');
    const html = buildHTML(sections);
    console.log('[OK] HTML generated');
    console.log();
